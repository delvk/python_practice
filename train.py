...
def main(args):
    ...
    with Graph.as_default():
        ...
        mse_bitwise = mcnn.loss(d_map_estimation, d_map_placeholder)
        ...
        with tf.Session(graph=Graph) as sess:
            ...
            for epoch in range(args.num_epochs):
                ...
                for img_idx in range(number_of_images):
                    ...
                    _, mse = sess.run([train_op, mse_bitwise], feed_dict=feed_dict)
                    et_map = sess.run(
                        d_map_estimation, feed_dict={image_placeholder: train_image}
                    )
                    loss_per_image = utils.loss_per_image(train_d_map, et_map)
                    print("mse: {:.4f}".format(mse))
                    print("gt_map: {:.4f} | et_map: {:.4f}".format(np.sum(train_d_map),np.sum(et_map)))
                ...
                log_print(temp_str, log_file=train_loss_log)
                ...
                for i in range(len(val_img_list)):
                    ...
                    val_et_map = sess.run(d_map_estimation, feed_dict=feed_dict_val)

                    # accumulate the validation loss across all the images.
                    val_total_loss += utils.loss_per_image(val_et_map, val_d_map)
                ...
                log_print(temp_str, log_file=val_loss_log)
                ...
                # --------------------Testing----------------------------------------------------------
                image = utils.load_image("/home/jake/Desktop/Projects/Python/dataset/SH_B/cooked/train_10/images/5_7.jpg")
                gt = utils.load_ground_truth("/home/jake/Desktop/Projects/Python/dataset/SH_B/cooked/train_10/ground_truth/5_7.csv", downsize=True)
                image = utils.reshape_tensor(image)
                # gt = utils.reshape_tensor(gt)
                ...
                print("MSE: {}".format(abs(gt_count - et_count)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session_id", default=4, type=int)
